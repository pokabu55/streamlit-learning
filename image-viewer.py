...
"""
Streamlitでは、スライダーの値が変更されると、全体のスクリプトが再実行されます。
しかし、ファイルアップローダーからのファイルは再実行時には保持されません。
そのため、スライダーを動かすと、アップロードされたファイルがなくなり、画像が更新されないという問題が発生します。

この問題を解決するためには、アップロードされたファイルを一時的に保存し、再実行時にその保存されたファイルを読み込むようにする必要があります。

以下にそのためのコードを示します。

このコードでは、アップロードされたファイルを一時的に"temp_img"という名前で保存します。
そして、そのファイルが存在する場合には、それを読み込み、画像の拡大縮小と表示を行います。
これにより、スライダーを動かしても画像が更新されるようになります。
